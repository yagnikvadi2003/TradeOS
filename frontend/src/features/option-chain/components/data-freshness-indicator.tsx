...
interface DataFreshnessIndicatorProps {
  /** Epoch ms of the oldest contract update in the current snapshot. */
...
export function DataFreshnessIndicator({
  ...
  const ageMs = Math.max(0, Math.max(tick, now()) - oldestUpdateAt);
  const freshness = classifyFreshness(ageMs);
  return (
    <span
      className={cn('inline-flex items-center gap-1.5 text-xs', className)}
      title={`${strings.optionChain.freshness.hint} ${formatIstTime(oldestUpdateAt)} IST`}
